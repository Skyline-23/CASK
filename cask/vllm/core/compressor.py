...
class TriAttentionCompressor:
    ...
    def _lazy_init(self):
        ...
        if self.config.stats_path is None:
            raise ValueError(
                ...
                "Generate statistics using: python -m cask.tools.generate_stats <model_path> <output_path>"
            )
        ...
        try:
            ...
            raise FileNotFoundError(
                ...
                f"Generate it using: python -m cask.tools.generate_stats <model_path> {self.config.stats_path}"
            )
        ...
        # Precompute per-layer scorer state for the selected horizon/norm modes.
        self._prepare_layer_scoring_state()
    ...
    def _prepare_layer_scoring_state(self):
        """Prepare per-layer tensors needed by the selected scorer mode."""
        if self.head_stats is None or self.omega is None or self.offsets is None:
            return
        for layer_idx, layer_data in self.head_stats.items():
            prepare_layer_scoring_state(
                ...
                horizon_mode=self.config.horizon_mode,
                norm_mode=self.config.norm_mode,
                disable_mlr=self.config.disable_mlr,
                kernel_c_lambda=self.config.kernel_c_lambda,
                kernel_s0=self.config.kernel_s0,
                kernel_s1=self.config.kernel_s1,
                norm_lambda=self.config.norm_lambda,
            )
