...
def _convert_rkv_stats(
    ...
    def _derive_inv_freq_fallback() -> Optional[torch.Tensor]:
        ...
        if model_id:
            try:
                ...
                from cask.common.rope_utils import build_rotary
    ...
    def _derive_freq_scale_sq_fallback() -> torch.Tensor:
        ...
        if model_id:
            try:
                from transformers import AutoConfig
                from cask.common.rope_utils import build_rotary, compute_frequency_scaling
    ...
    for layer_idx in sorted(layer_nums):
        ...
        all_oracle_tau_imag = []
        all_oracle_pi = []
        all_oracle_horizon_mean_real = []
        all_oracle_horizon_mean_imag = []
        has_q_sq_abs_mean = True
        has_oracle_tau = True
        has_oracle_pi = True
        has_oracle_horizon_mean = True
        ...
        for head_idx in sorted(head_nums):
            ...
            if key in rkv_stats:
                ...
                if "q_sq_abs_mean" in head_data:
                    q_sq_abs = head_data["q_sq_abs_mean"].to(dtype=dtype)
                    all_q_sq_abs_mean.append(q_sq_abs)
                else:
                    has_q_sq_abs_mean = False

                if "oracle_tau_real" in head_data and "oracle_tau_imag" in head_data:
                    all_oracle_tau_real.append(head_data["oracle_tau_real"].to(dtype=dtype))
                    all_oracle_tau_imag.append(head_data["oracle_tau_imag"].to(dtype=dtype))
                else:
                    has_oracle_tau = False

                if "oracle_pi" in head_data:
                    all_oracle_pi.append(head_data["oracle_pi"].to(dtype=dtype))
                else:
                    has_oracle_pi = False

                if "oracle_horizon_mean_real" in head_data and "oracle_horizon_mean_imag" in head_data:
                    all_oracle_horizon_mean_real.append(
                        head_data["oracle_horizon_mean_real"].to(dtype=dtype)
                    )
                    all_oracle_horizon_mean_imag.append(
                        head_data["oracle_horizon_mean_imag"].to(dtype=dtype)
                    )
                else:
                    has_oracle_horizon_mean = False
        ...
        if has_q_sq_abs_mean and all_q_sq_abs_mean:
            stacked_q_sq_abs = torch.stack(all_q_sq_abs_mean, dim=0)
            if gqa_ratio > 1:
                q_sq_abs_mean = stacked_q_sq_abs.reshape(
                    num_kv_heads, gqa_ratio, freq_count
                ).mean(dim=1)
            else:
                q_sq_abs_mean = stacked_q_sq_abs
            head_stats[layer_idx]["q_sq_abs_mean"] = q_sq_abs_mean.to(device)

        if has_oracle_tau and all_oracle_tau_real and all_oracle_tau_imag:
            stacked_oracle_tau_real = torch.stack(all_oracle_tau_real, dim=0)
            stacked_oracle_tau_imag = torch.stack(all_oracle_tau_imag, dim=0)
            if gqa_ratio > 1:
                oracle_tau_real = stacked_oracle_tau_real.reshape(
                    num_kv_heads, gqa_ratio, freq_count
                ).mean(dim=1)
                oracle_tau_imag = stacked_oracle_tau_imag.reshape(
                    num_kv_heads, gqa_ratio, freq_count
                ).mean(dim=1)
            else:
                oracle_tau_real = stacked_oracle_tau_real
                oracle_tau_imag = stacked_oracle_tau_imag
            head_stats[layer_idx]["oracle_tau_complex"] = torch.stack(
                [oracle_tau_real, oracle_tau_imag],
                dim=-1,
            ).to(device)

        if has_oracle_pi and all_oracle_pi:
            stacked_oracle_pi = torch.stack(all_oracle_pi, dim=0)
            if gqa_ratio > 1:
                oracle_pi = stacked_oracle_pi.reshape(
                    num_kv_heads, gqa_ratio, stacked_oracle_pi.shape[-1]
                ).mean(dim=1)
            else:
                oracle_pi = stacked_oracle_pi
            head_stats[layer_idx]["oracle_pi"] = oracle_pi.to(device)

        if has_oracle_horizon_mean and all_oracle_horizon_mean_real and all_oracle_horizon_mean_imag:
            stacked_oracle_horizon_mean_real = torch.stack(all_oracle_horizon_mean_real, dim=0)
            stacked_oracle_horizon_mean_imag = torch.stack(all_oracle_horizon_mean_imag, dim=0)
            if gqa_ratio > 1:
                oracle_horizon_mean_real = stacked_oracle_horizon_mean_real.reshape(
                    num_kv_heads, gqa_ratio, freq_count
                ).mean(dim=1)
                oracle_horizon_mean_imag = stacked_oracle_horizon_mean_imag.reshape(
                    num_kv_heads, gqa_ratio, freq_count
                ).mean(dim=1)
            else:
                oracle_horizon_mean_real = stacked_oracle_horizon_mean_real
                oracle_horizon_mean_imag = stacked_oracle_horizon_mean_imag
            head_stats[layer_idx]["oracle_horizon_mean_complex"] = torch.stack(
                [oracle_horizon_mean_real, oracle_horizon_mean_imag],
                dim=-1,
            ).to(device)
...
def prepare_layer_scoring_state(
    *,
    ...
    norm_mode: str,
    disable_mlr: bool,
    kernel_c_lambda: float,
    kernel_s0: float,
    kernel_s1: float,
    norm_lambda: float,
) -> None:
    """Augment per-layer stats with prepared scorer tensors.

    Adds:
    - ``horizon_mean_complex``: [num_heads, freq_count, 2]
    - ``extra_coef``: [num_heads, freq_count]
    """
    q_mean_complex_raw = layer_data.get("q_mean_complex")
    q_abs_mean = layer_data.get("q_abs_mean")
    if not isinstance(q_mean_complex_raw, torch.Tensor) or not isinstance(q_abs_mean, torch.Tensor):
        raise ValueError("Layer stats must contain q_mean_complex and q_abs_mean")

    q_mean_complex = torch.complex(
        q_mean_complex_raw[..., 0].to(dtype=torch.float32),
        q_mean_complex_raw[..., 1].to(dtype=torch.float32),
    )
    q_abs_mean_fp32 = q_abs_mean.to(dtype=torch.float32)
    q_mean_abs = torch.abs(q_mean_complex).to(dtype=torch.float32)

    if horizon_mode == "adaptive":
        kappa_values = []
        for head_idx in range(q_mean_complex.shape[0]):
            _, _, _, _, kappa_complex = build_adaptive_horizon_kernel(
                q_mean_complex[head_idx],
                q_abs_mean_fp32[head_idx],
                omega,
                offsets,
                c_lambda=kernel_c_lambda,
                s0=kernel_s0,
                s1=kernel_s1,
            )
            kappa_values.append(kappa_complex.to(dtype=torch.complex64))
        horizon_mean_complex = torch.stack(kappa_values, dim=0)
    elif horizon_mode == "variational":
        oracle_horizon_mean_raw = layer_data.get("oracle_horizon_mean_complex")
        if not isinstance(oracle_horizon_mean_raw, torch.Tensor):
            raise ValueError(
                "oracle_horizon_mean_complex is required for horizon_mode='variational'"
            )
        horizon_mean_complex = torch.complex(
            oracle_horizon_mean_raw[..., 0].to(dtype=torch.float32),
            oracle_horizon_mean_raw[..., 1].to(dtype=torch.float32),
        )
    else:
        offset_mean_complex = compute_offset_mean_complex(offsets, omega).to(dtype=torch.complex64)
        horizon_mean_complex = offset_mean_complex.unsqueeze(0).expand(q_mean_complex.shape[0], -1)

    if norm_mode == "tri":
        alpha_tri = (q_abs_mean_fp32 - q_mean_abs).clamp_min(0.0)
        extra_coef = q_abs_mean_fp32 if disable_mlr else alpha_tri
    else:
        q_sq_abs_mean = layer_data.get("q_sq_abs_mean")
        if not isinstance(q_sq_abs_mean, torch.Tensor):
            raise ValueError("q_sq_abs_mean is required for norm_mode='rms2'")
        extra_coef = compute_rms2_coefficient(
            q_mean_complex,
            q_abs_mean_fp32,
            q_sq_abs_mean.to(dtype=torch.float32),
        )

    extra_coef = extra_coef.to(dtype=torch.float32) * float(norm_lambda)
    layer_data["horizon_mean_complex"] = torch.stack(
        [horizon_mean_complex.real, horizon_mean_complex.imag],
        dim=-1,
    ).to(device=q_mean_complex_raw.device, dtype=torch.float32)
    layer_data["extra_coef"] = extra_coef.to(device=q_mean_complex_raw.device, dtype=torch.float32)
