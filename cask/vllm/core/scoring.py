...
def _supports_triton_scoring(config: TriAttentionConfig) -> bool:
    return (
        config.use_triton_scoring
        and config.horizon_mode == "fixed"
        and config.norm_mode == "tri"
        and not config.disable_mlr
        and not config.disable_trig
    )
...
def compute_scores(
    ...
    """
    if _supports_triton_scoring(config):
...
def compute_scores_triton(
    ...
    """
    from .kernels.triton_scoring import cask_scoring
...
def compute_scores_pytorch(
    ...
    The derivation is documented in the inline comments and the Triton kernel notes.
    ...
    q_mean_complex_raw = head_stats["q_mean_complex"]  # [num_kv_heads, freq_count, 2]
    ...
    q_abs_mean = head_stats.get("q_abs_mean")
    if isinstance(q_abs_mean, torch.Tensor):
        q_abs_mean = q_abs_mean.to(dtype=torch.float32)
    else:
        q_abs_mean = q_mean_abs

    extra_coef = head_stats.get("extra_coef")
    if isinstance(extra_coef, torch.Tensor):
        extra_coef = extra_coef.to(dtype=torch.float32)
    else:
        extra_coef = q_abs_mean if config.disable_mlr else (q_abs_mean - q_mean_abs).clamp_min(0.0)

    horizon_mean_complex_raw = head_stats.get("horizon_mean_complex")
    horizon_mean_complex = None
    if isinstance(horizon_mean_complex_raw, torch.Tensor):
        horizon_mean_complex = torch.complex(
            horizon_mean_complex_raw[..., 0].to(dtype=torch.float32),
            horizon_mean_complex_raw[..., 1].to(dtype=torch.float32),
        )

    if config.rope_style == "interleaved":
        k_pairs = key_states.reshape(batch_size, num_kv_heads, seq_len, freq_count, 2).to(dtype=torch.float32)
        k_complex = torch.complex(k_pairs[..., 0], k_pairs[..., 1])
    else:
        half_dim = head_dim // 2
        k_real = key_states[..., :half_dim].to(dtype=torch.float32)
        k_imag = key_states[..., half_dim:].to(dtype=torch.float32)
        k_complex = torch.complex(k_real, k_imag)
    k_abs = torch.abs(k_complex)
    conj_k = torch.conj(k_complex)
    freq_scale_expanded = freq_scale_sq.to(dtype=torch.float32).unsqueeze(0).unsqueeze(2)
    ...
    extra_term = (
        k_abs * extra_coef.unsqueeze(0).unsqueeze(2) * freq_scale_expanded
    ).sum(dim=-1)  # [batch, num_kv_heads, seq_len]

    if config.disable_trig:
        scores = extra_term
    elif config.score_aggregation == "mean":
        if horizon_mean_complex is None:
            phases = offsets.to(dtype=torch.float32).unsqueeze(1) * omega.to(dtype=torch.float32).unsqueeze(0)
            horizon_mean_complex = torch.polar(torch.ones_like(phases), phases).mean(dim=0)
            horizon_mean_complex = horizon_mean_complex.unsqueeze(0).expand(num_kv_heads, -1)
        round_phase = torch.polar(
            torch.ones_like(omega, dtype=torch.float32),
            float(round_start) * omega.to(dtype=torch.float32),
        )
        gamma = q_mean_complex * horizon_mean_complex.to(device=key_states.device) * round_phase
        base_scores = (
            (gamma.unsqueeze(0).unsqueeze(2) * conj_k).real * freq_scale_expanded
        ).sum(dim=-1)
        scores = base_scores + extra_term
    elif config.score_aggregation == "max":
        offset_positions = offsets.to(dtype=torch.float32) + float(round_start)
        phases = offset_positions.unsqueeze(1) * omega.to(dtype=torch.float32).unsqueeze(0)
        round_phases = torch.polar(torch.ones_like(phases), phases)
        weighted_q = q_mean_complex.unsqueeze(0) * round_phases.unsqueeze(1)
        base_scores = (
            (weighted_q.unsqueeze(2) * conj_k.unsqueeze(0)).real
            * freq_scale_expanded.unsqueeze(0)
        ).sum(dim=-1)
        scores = base_scores.max(dim=0).values + extra_term
    else:
