...
def resolve_group_tensors(base_runner: Any) -> dict[int, list[tuple[int, torch.Tensor]]]:
    ...
    for gid, group in enumerate(kv_cache_groups):
        ...
        for local_idx, layer_name in enumerate(layer_names):
            ...
            if layer is None:
                continue
            kv_cache_value = getattr(layer, "kv_cache", None)
            if isinstance(kv_cache_value, torch.Tensor):
                tensor = kv_cache_value
            elif isinstance(kv_cache_value, (list, tuple)) and kv_cache_value:
                tensor = kv_cache_value[0]
            else:
                continue
