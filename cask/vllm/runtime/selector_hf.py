...
def build_triattention_selector(
    ...
    try:
        from cask.vllm.core.config import TriAttentionConfig
        from cask.vllm.core.compressor import TriAttentionCompressor
        from cask.vllm.core.scoring import compute_scores
        from cask.vllm.core.utils import normalize_scores
    ...
    tri_cfg = TriAttentionConfig(
        ...
        horizon_mode=config.sparse_horizon_mode,
        norm_mode=config.sparse_norm_mode,
        kernel_c_lambda=config.sparse_kernel_c_lambda,
        kernel_s0=config.sparse_kernel_s0,
        kernel_s1=config.sparse_kernel_s1,
        norm_lambda=config.sparse_norm_lambda,
        adaptive_mean_only=config.sparse_adaptive_mean_only,
    ...
    try:
        ...
        raise RuntimeError(
            f"{TRITON_SCORING_REQUIRED_MARKER}:stats_init_failed:{type(exc).__name__}:{exc}"
        ) from exc
    ...
    def _build_reduced_layer_stats(
        *,
        ...
        for key in (
            ...
            if value.ndim == 2:
                reduced_stats[key] = (
                    value.reshape(target_heads, group_size, value.shape[1])
                    .mean(dim=1)
                    .contiguous()
                )
            elif value.ndim == 3:
