...
class TriAttentionRuntimeConfig:
    ...
    sparse_horizon_mode: str = "fixed"
    sparse_norm_mode: str = "tri"
    sparse_kernel_c_lambda: float = 1.0
    sparse_kernel_s0: float = 1.0
    sparse_kernel_s1: float = 1.0
    sparse_norm_lambda: float = 1.0
    sparse_adaptive_mean_only: bool = True
    ...
    def from_env(cls, prefix: str = "TRIATTN_RUNTIME_") -> "TriAttentionRuntimeConfig":
        ...
        config = cls(
            ...
            sparse_horizon_mode=(
                maybe_str("SPARSE_HORIZON_MODE", cls.sparse_horizon_mode)
                or cls.sparse_horizon_mode
            ),
            sparse_norm_mode=(
                maybe_str("SPARSE_NORM_MODE", cls.sparse_norm_mode)
                or cls.sparse_norm_mode
            ),
            sparse_kernel_c_lambda=maybe_float(
                "SPARSE_KERNEL_C_LAMBDA", cls.sparse_kernel_c_lambda
            ),
            sparse_kernel_s0=maybe_float(
                "SPARSE_KERNEL_S0", cls.sparse_kernel_s0
            ),
            sparse_kernel_s1=maybe_float(
                "SPARSE_KERNEL_S1", cls.sparse_kernel_s1
            ),
            sparse_norm_lambda=maybe_float(
                "SPARSE_NORM_LAMBDA", cls.sparse_norm_lambda
            ),
            sparse_adaptive_mean_only=maybe_bool(
                "SPARSE_ADAPTIVE_MEAN_ONLY", cls.sparse_adaptive_mean_only
            ),
    ...
    def validate(self) -> None:
        ...
        if self.sparse_horizon_mode not in {"fixed", "adaptive", "variational"}:
            raise ValueError(
                "sparse_horizon_mode must be 'fixed', 'adaptive', or 'variational', "
                f"got {self.sparse_horizon_mode!r}"
            )
        if self.sparse_norm_mode not in {"tri", "rms2"}:
            raise ValueError(
                "sparse_norm_mode must be 'tri' or 'rms2', "
                f"got {self.sparse_norm_mode!r}"
            )
        if (
            self.sparse_adaptive_mean_only
            and self.sparse_horizon_mode in {"adaptive", "variational"}
            and self.sparse_score_aggregation != "mean"
        ):
            raise ValueError(
                f"{self.sparse_horizon_mode} sparse_horizon_mode currently supports sparse_score_aggregation='mean' only"
            )
