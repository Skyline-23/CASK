...
from cask.methods.baselines import (
    ExpectedAttention,
    R1KV,
    SnapKV,
)
...
def _append_query_cache(
    ...
    *,
    ...
    if not hasattr(past_key_value, cache_name):
        setattr(past_key_value, cache_name, {})
    cache_dict = getattr(past_key_value, cache_name)

    if layer_idx not in cache_dict:
        bsz, n_heads, _, head_dim = query_states.shape
        cache_dict[layer_idx] = torch.empty(
            bsz,
            n_heads,
            0,
            head_dim,
        ...
        cache_dict[layer_idx] = query_states[:, :, -window_size:, :]
    else:
        cache_dict[layer_idx] = torch.cat((cache_dict[layer_idx], query_states), dim=2)
        if cache_dict[layer_idx].shape[-2] > window_size:
            cache_dict[layer_idx] = cache_dict[layer_idx][:, :, -window_size:, :]
    return cache_dict[layer_idx]
...
def LlamaAttention_forward(
    ...
    **kwargs: Unpack[FlashAttentionKwargs],
    ...
    query_states_prerope = self.q_proj(hidden_states).view(hidden_shape).transpose(1, 2)
    key_states = self.k_proj(hidden_states).view(hidden_shape).transpose(1, 2)
    value_states = self.v_proj(hidden_states).view(hidden_shape).transpose(1, 2)

    cos, sin = position_embeddings
    query_states, key_states = apply_rotary_pos_emb(query_states_prerope, key_states, cos, sin)
    ...
    if past_key_value is not None:
        ...
        cached_queries = _append_query_cache(
            past_key_value,
        ...
        cached_queries_prerope = None
        if getattr(self.kv_cluster, "requires_prerope_queries", False):
            cached_queries_prerope = _append_query_cache(
                past_key_value,
                layer_idx=self.layer_idx,
                cache_name="query_cache_prerope",
                query_states=query_states_prerope,
                window_size=window_size,
            )
...
def Qwen3Attention_forward(
    ...
    **kwargs: Unpack[FlashAttentionKwargs],
    ...
    query_states_prerope = self.q_norm(self.q_proj(hidden_states).view(hidden_shape)).transpose(1, 2)
