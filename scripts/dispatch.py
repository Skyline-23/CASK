...
def resolve_python_command(conda_env: str | None, experiment: Dict[str, object]) -> List[str]:
    explicit_python = experiment.get("python_executable") or os.environ.get("TRIATTN_PYTHON_EXECUTABLE")
    if explicit_python:
        return [str(explicit_python)]
    if conda_env and shutil.which("conda"):
        return ["conda", "run", "-n", conda_env, "python"]
    if conda_env:
        print(f"[warn] conda not found; falling back to current python: {sys.executable}")
    return [sys.executable]


def build_base_command(python_cmd: List[str], runner_path: Path, runner_args: List[str]) -> List[str]:
    return python_cmd + [str(runner_path)] + runner_args
...
def run_evaluation(base_dir: Path, dataset: str, exp_name: str, output_dir: Optional[Path], python_cmd: List[str], dry_run: bool, num_samples: int | None = None) -> None:
...
def main() -> None:
    ...
    python_cmd = resolve_python_command(conda_env, experiment)
    ...
    eval_output_dir_value = args.eval_output_dir or experiment.get("eval_output_dir")
    eval_output_dir = resolve_path(eval_output_dir_value) if eval_output_dir_value else None
    skip_eval = args.no_eval or bool(experiment.get("no_eval", False))
    ...
    base_cmd = build_base_command(python_cmd, runner_path, format_runner_args(runner_args, total_shards))
    ...
    if not skip_eval:
        ...
        run_evaluation(merged_dir, args.dataset, exp_name, eval_output_dir, python_cmd, args.dry_run, num_samples)
