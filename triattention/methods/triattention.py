...
def _cache_to_legacy_tuple(pkv: Cache | Sequence[Tuple[torch.Tensor, torch.Tensor]] | None) -> Tuple[Tuple[torch.Tensor, torch.Tensor], ...]:
    """Normalize cache objects across transformers cache API versions."""
    if pkv is None:
        return ()
    if isinstance(pkv, Cache):
        if hasattr(pkv, "to_legacy_cache"):
            return pkv.to_legacy_cache()  # type: ignore[return-value]
        ...
        for layer in layers:
            if hasattr(layer, "keys") and hasattr(layer, "values"):
                normalized.append((layer.keys, layer.values))
                continue
            if hasattr(layer, "key_cache") and hasattr(layer, "value_cache"):
                normalized.append((layer.key_cache, layer.value_cache))
                continue
            raise AttributeError(f"Unsupported cache layer type: {type(layer)!r}")
        return tuple(normalized)

    return tuple(pkv) if pkv else ()


def _legacy_tuple_to_cache(
    pkv_tuple: Tuple[Tuple[torch.Tensor, torch.Tensor], ...],
    template_cache: Cache | Sequence[Tuple[torch.Tensor, torch.Tensor]] | None,
) -> Cache | Tuple[Tuple[torch.Tensor, torch.Tensor], ...]:
    """Rebuild a cache object using the active transformers cache API."""
    if not isinstance(template_cache, Cache):
        return pkv_tuple

    if hasattr(DynamicCache, "from_legacy_cache"):
        return DynamicCache.from_legacy_cache(pkv_tuple)  # type: ignore[return-value]
...
class TriAttention:
    ...
    def __init__(self, config: TriAttentionConfig) -> None:
        ...
        self.offset_mean_complex = compute_offset_mean_complex(self.offsets, self.omega)
        ...
        self.horizon_mode = config.horizon_mode
        self.norm_mode = config.norm_mode
        self.kernel_c_lambda = float(config.kernel_c_lambda)
        self.kernel_s0 = float(config.kernel_s0)
        self.kernel_s1 = float(config.kernel_s1)
        self.norm_lambda = float(config.norm_lambda)
        self.adaptive_mean_only = bool(config.adaptive_mean_only)
        self.allow_prefill_compression = config.allow_prefill_compression
        self.score_dump_dir = Path(config.score_dump_dir) if config.score_dump_dir else None
        self.score_dump_max_events = config.score_dump_max_events
        self._score_dump_event_index = 0
        self._score_dump_context: Dict[str, object] = {}
        if self.score_dump_dir is not None:
            self.score_dump_dir.mkdir(parents=True, exist_ok=True)
        ...
        if self.horizon_mode not in {"fixed", "adaptive", "variational"}:
            raise ValueError(f"Unsupported horizon_mode: {self.horizon_mode}")
        if self.norm_mode not in {"tri", "rms2"}:
            raise ValueError(f"Unsupported norm_mode: {self.norm_mode}")
        if self.norm_mode != "tri" and self.disable_mlr:
            raise ValueError("disable_mlr is only supported with norm_mode='tri'")
        if (
            self.horizon_mode in {"adaptive", "variational"}
            and self.adaptive_mean_only
            and self.score_aggregation != "mean"
        ):
            raise ValueError(
                f"{self.horizon_mode} horizon_mode currently supports score_aggregation='mean' only"
            )

        self.head_alpha_tri: Dict[Tuple[int, int], torch.Tensor] = {}
        self.head_alpha_rms2: Dict[Tuple[int, int], torch.Tensor] = {}
        self.head_R_band: Dict[Tuple[int, int], torch.Tensor] = {}
        self.head_lambda_eff: Dict[Tuple[int, int], torch.Tensor] = {}
        self.head_R_bar: Dict[Tuple[int, int], torch.Tensor] = {}
        self.head_pi: Dict[Tuple[int, int], torch.Tensor] = {}
        self.head_horizon_mean_complex: Dict[Tuple[int, int], torch.Tensor] = {}
        self.head_extra_coef: Dict[Tuple[int, int], torch.Tensor] = {}
        self._initialize_head_scoring_state()

    def _initialize_head_scoring_state(self) -> None:
        for key, stats in self.head_stats.items():
            q_mean_abs = torch.abs(stats.q_mean_complex).to(device=self.config.device, dtype=torch.float32)
            alpha_tri = (stats.q_abs_mean.to(device=self.config.device, dtype=torch.float32) - q_mean_abs).clamp_min(0.0)
            self.head_alpha_tri[key] = alpha_tri
            self.head_R_band[key] = compute_band_concentration(
                stats.q_mean_complex,
                stats.q_abs_mean,
            )

            if stats.q_sq_abs_mean is not None:
                self.head_alpha_rms2[key] = compute_rms2_coefficient(
                    stats.q_mean_complex,
                    stats.q_abs_mean,
                    stats.q_sq_abs_mean,
                )

            if self.horizon_mode == "adaptive":
                r_band, lambda_eff, r_bar, pi, kappa_complex = build_adaptive_horizon_kernel(
                    stats.q_mean_complex,
                    stats.q_abs_mean,
                    self.omega,
                    self.offsets,
                    c_lambda=self.kernel_c_lambda,
                    s0=self.kernel_s0,
                    s1=self.kernel_s1,
                )
                self.head_R_band[key] = r_band
                self.head_lambda_eff[key] = lambda_eff
                self.head_R_bar[key] = r_bar
                self.head_pi[key] = pi
                self.head_horizon_mean_complex[key] = kappa_complex
            elif self.horizon_mode == "variational":
                if stats.oracle_horizon_mean_complex is None:
                    raise ValueError(
                        f"Stats file {self.config.stats_path} is missing oracle_horizon_mean_complex for head {key}. "
                        "Regenerate stats with scripts/calibrate.py --build-variational-horizon to use "
                        "horizon_mode='variational'."
                    )
                if stats.oracle_pi is not None:
                    self.head_pi[key] = stats.oracle_pi.to(device=self.config.device, dtype=torch.float32)
                self.head_horizon_mean_complex[key] = stats.oracle_horizon_mean_complex.to(
                    device=self.config.device,
                    dtype=torch.complex64,
                )
            else:
                self.head_horizon_mean_complex[key] = self.offset_mean_complex

            if self.norm_mode == "tri":
                selected_extra = stats.q_abs_mean if self.disable_mlr else alpha_tri
                self.head_extra_coef[key] = selected_extra.to(device=self.config.device, dtype=torch.float32) * self.norm_lambda
                continue

            alpha_rms2 = self.head_alpha_rms2.get(key)
            if alpha_rms2 is None:
                raise ValueError(
                    f"Stats file {self.config.stats_path} is missing q_sq_abs_mean for head {key}. "
                    "Regenerate stats with scripts/calibrate.py to use norm_mode='rms2'."
                )
            self.head_extra_coef[key] = alpha_rms2.to(device=self.config.device, dtype=torch.float32) * self.norm_lambda

    def set_dump_context(self, **context: object) -> None:
        normalized: Dict[str, object] = {}
        for key, value in context.items():
            if key in {"shard_id", "run_id", "sample_idx", "prefill_length"}:
                normalized[key] = int(value)
            else:
                normalized[key] = value
        self._score_dump_context = normalized
        self._score_dump_event_index = 0

    def clear_dump_context(self) -> None:
        self._score_dump_context = {}
        self._score_dump_event_index = 0

    def _dump_score_event(
        self,
        *,
        raw_head_matrix: torch.Tensor,
        scored_head_matrix: torch.Tensor,
        combined_scores: Optional[torch.Tensor],
        ...
        payload = {
            "metadata": {
                ...
                "count_prompt_tokens": bool(self.config.count_prompt_tokens),
                "allow_prefill_compression": bool(self.allow_prefill_compression),
                "per_head_pruning": bool(self.per_head_pruning),
                "per_layer_perhead_pruning": bool(self.per_layer_perhead_pruning),
                "disable_mlr": bool(self.disable_mlr),
                "disable_trig": bool(self.disable_trig),
                "horizon_mode": self.horizon_mode,
                "norm_mode": self.norm_mode,
                "kernel_c_lambda": float(self.kernel_c_lambda),
                "kernel_s0": float(self.kernel_s0),
                "kernel_s1": float(self.kernel_s1),
                "norm_lambda": float(self.norm_lambda),
                "sampled_heads": [list(item) for item in self.sampled_heads],
                **context,
            },
            "tensors": {
                "decode_positions": decode_positions.detach().to("cpu"),
                "raw_head_matrix": raw_head_matrix.detach().to("cpu"),
                "scored_head_matrix": scored_head_matrix.detach().to("cpu"),
                "keep_indices": keep_indices.detach().to("cpu"),
            },
        }
        if combined_scores is not None:
            payload["tensors"]["combined_scores"] = combined_scores.detach().to("cpu")

        torch.save(payload, self.score_dump_dir / file_name)
    ...
    def compute_keep_indices(
        ...
        raw_head_matrix = torch.cat(all_head_scores, dim=0)
        head_matrix = raw_head_matrix.clone()
        ...
        combined_scores: Optional[torch.Tensor] = None

        if self.per_layer_perhead_pruning:
            selection_mode = "per_layer_perhead"
            keep_count = min(decode_budget, decode_count)
            keep_indices = self._select_per_layer_perhead_independent(
                pkv_tuple, keep_count, decode_start, decode_positions
            )
            self._dump_score_event(
                raw_head_matrix=raw_head_matrix,
                scored_head_matrix=head_matrix,
        ...
        if self.per_head_pruning:
            ...
            keep_count = min(decode_budget, decode_count)
            keep_indices = self._select_per_head_independent(
        ...
        combined_scores = head_matrix.max(dim=0).values

        # Union-based selection
        keep_count = min(decode_budget, decode_count)
        decode_keep_indices = self._select_union_based(head_matrix, combined_scores, keep_count)
    ...
    def _compute_layer_head_scores(
        ...
        Args:
            ...
            positions_per_kv_head: Optional per-KV-head positions [num_kv_heads][seq_len].
                                   Retained for signature compatibility. Rotated KV cache states
                                   already encode key positions, so scoring no longer needs them.
        ...
        for layer, head in layer_heads:
            ...
            # Gather rotated keys directly from the cache. RoPE already encodes
            # the key position, so no per-token de-rotation or position loads are
            # needed in the scoring hot path.
            k_rot = key_states[0, kv_head].index_select(0, gather_indices)
            k_rot = k_rot.to(device=self.config.device, dtype=self.config.dtype)

            head_scores = score_rotated_keys_for_round(
                k_rot=k_rot,
                ...
                extra_coef=self.head_extra_coef[(layer, head)],
                horizon_mean_complex=self.head_horizon_mean_complex[(layer, head)] if self.score_aggregation == "mean" else None,
                offset_mean_complex=self.offset_mean_complex if self.score_aggregation == "mean" else None,
                offsets=None if self.score_aggregation == "mean" else self.offsets,
...
def apply_triattention_patch(
    ...
    *,
    ...
    config = TriAttentionConfig(
        ...
        norm_mode=norm_mode,
        kernel_c_lambda=kernel_c_lambda,
        kernel_s0=kernel_s0,
        kernel_s1=kernel_s1,
        norm_lambda=norm_lambda,
        adaptive_mean_only=adaptive_mean_only,
