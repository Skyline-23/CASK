...
class TriAttentionConfig:
    ...
    horizon_mode: Literal["fixed", "adaptive", "variational"] = "fixed"
    """Future horizon kernel mode."""

    norm_mode: Literal["tri", "rms2"] = "tri"
    """Norm coefficient mode."""

    kernel_c_lambda: float = 1.0
    """Adaptive horizon kernel center multiplier."""

    kernel_s0: float = 1.0
    """Adaptive horizon kernel base width."""

    kernel_s1: float = 1.0
    """Adaptive horizon kernel concentration-dependent width term."""

    norm_lambda: float = 1.0
    """Scalar multiplier applied to the selected norm coefficient."""

    adaptive_mean_only: bool = True
    """Restrict adaptive horizon mode to mean aggregation."""
    ...
    def __post_init__(self):
        ...
        if self.horizon_mode not in {"fixed", "adaptive", "variational"}:
            raise ValueError(
                f"horizon_mode must be 'fixed', 'adaptive', or 'variational', got {self.horizon_mode!r}"
            )

        if self.norm_mode not in {"tri", "rms2"}:
            raise ValueError(
                f"norm_mode must be 'tri' or 'rms2', got {self.norm_mode!r}"
            )

        if self.adaptive_mean_only and self.horizon_mode in {"adaptive", "variational"} and self.score_aggregation != "mean":
            raise ValueError(
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "kernel_c_lambda": self.kernel_c_lambda,
            "kernel_s0": self.kernel_s0,
            "kernel_s1": self.kernel_s1,
            "norm_lambda": self.norm_lambda,
            "adaptive_mean_only": self.adaptive_mean_only,
