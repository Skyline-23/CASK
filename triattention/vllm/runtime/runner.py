...
class TriAttentionModelRunner:
    ...
    def __init__(self, base_runner: Any, config: TriAttentionRuntimeConfig | None = None):
        ...
        self._allowed_strict_skip_reasons = {
            "under_budget",
            # vLLM V1 async scheduling can surface an early step before mutable
            # block groups are visible on the worker. Retrying on the next step
            # preserves strictness for real compaction failures without killing
            # otherwise valid long-context requests.
