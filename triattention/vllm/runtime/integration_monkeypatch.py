...
def _patched_engine_core_step_with_batch_queue(self):
    ...
    if self.scheduler.has_requests() and not boundary_pending:
        ...
        ec_consumer = getattr(self, "is_ec_consumer", None)
        if ec_consumer is None:
            ec_producer = getattr(self, "is_ec_producer", None)
            ec_consumer = True if ec_producer is None else (not bool(ec_producer))
        if ec_consumer:
