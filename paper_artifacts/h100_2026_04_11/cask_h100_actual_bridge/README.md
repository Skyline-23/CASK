...
This directory packages the H100 output-level evidence that complements the replay-fidelity gates in `paper_artifacts/h100_2026_04_10/cask_h100_fidelity/`.
...
Headline read:
- `qasper`: `CASK @ 256` crosses above `TriAttention @ 512` on actual output similarity and task metric.
- `multi_news`: `CASK @ 384` beats `TriAttention @ 384` on actual output similarity and task metric at the same budget.
